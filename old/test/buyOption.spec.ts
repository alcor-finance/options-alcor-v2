...
type SellingLimitOrder = {
    owner: string,
    for_buying: boolean,
    contracts_amount: BigNumber,
    premiumTick: string,
    deadline: string
}

let [account1, account2, account3] = [] as Wallet[]
let SELLING_LIMIT_ORDERS = [] as SellingLimitOrder[]

(ethers as any).getSigners().then((signers: Wallet[]) => {
    account1 = signers[0]
    account2 = signers[1]
    account3 = signers[2]


    SELLING_LIMIT_ORDERS = [
        {
            owner: account1.address, // dev 1
            for_buying: false,
