...
library BS {
    ...
    function D1(
        ...
        int d1 = ((S.div(K)).ln() + (r + ((sigma.pow(2e18)).div(2e18)).mul(T)))
    ...
    /// @param T expiry
    /// @return d2 d2
    function D2(int d1, int sigma, int T) internal pure returns (int) {
    ...
    /// @return delta delta
    function BS_CALL(
        BlackScholesInput memory _params
    ) internal pure returns (int C, int delta) {
        int d1 = D1(_params.S, _params.K, _params.r, _params.sigma, _params.T);
        int d2 = D2(d1, _params.sigma, _params.T);
        C =
            _params.S.mul(Statistics.cdf(d1)) -
            (
                _params.K.mul((-_params.r.mul(_params.T)).exp()).mul(
                    Statistics.cdf(d2)
                )
            );
